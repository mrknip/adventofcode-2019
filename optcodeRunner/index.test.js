...
const {runOpcodeProgram, runGravityAssist, getNounVerbForTarget} = require('./index');

describe('runOpcodeProgram', () => {
  ...
  it('works - puzzle case', () => {
    ...
    const result = runOpcodeProgram(puzzle_input);
  ...
  it('processes optcode 3 - set from input', () => {
    ...
    const input = 9000;
    const result = runOpcodeProgram(program, input);
  ...
  it('handles comparison', () => {
    ...
    const result = runOpcodeProgram(program, 1);
    expect(result).to.equal(`0`);
    expect(runOpcodeProgram(program, 8)).to.equal(`1`);
  });

  it('handles less than', () => {
    const program = [3,9,7,9,10,9,4,9,99,-1,8];
    expect(runOpcodeProgram(program, 1)).to.equal(`1`);
    expect(runOpcodeProgram(program, 8)).to.equal(`0`);
  });

  it('handles equal to (immediate)', () => {
    const program = [3,3,1108,-1,8,3,4,3,99];
    expect(runOpcodeProgram(program, 1)).to.equal(`0`);
    expect(runOpcodeProgram(program, 8)).to.equal(`1`);
  });

  it('handles jump if true', () => {
    const program = [3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9];
    expect(runOpcodeProgram(program, 0)).to.equal(`0`);
    expect(runOpcodeProgram(program, 8)).to.equal(`1`);
  });
...
999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99];
    expect(runOpcodeProgram(program, 0)).to.equal(`999`);
    expect(runOpcodeProgram(program, 8)).to.equal(`1000`);
    expect(runOpcodeProgram(program, 9)).to.equal(`1001`);
  });
...
describe('AOC_5 - diagnostics', () => {
  it('works', () => {
    ...
    const userInput = 1;
    const result = runOpcodeProgram(program, userInput);
